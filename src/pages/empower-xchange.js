...
const EmpXchange = props => {
  ...
  const {empHeader,empProducts,empModules,empModuleArray} = FetchedEmpowerFoundationPageData(data?.allEmpPage?.nodes[0]?.journey[2]?.attributes?.dynamicCmp);
  const imgAlt = "enterprise ai"

  return (
    <>
      {empHeader && (
        <div className={`platform-v3  emPower-xChange emPower-page ${isNavBarOpen ? "" : "no-scroll"}`}>
